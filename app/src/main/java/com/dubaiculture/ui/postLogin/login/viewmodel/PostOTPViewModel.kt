...
@HiltViewModel
class PostOTPViewModel @Inject constructor(
                ...
                when (val result = registrationRepository.validateOTP(it)) {
                    is Result.Success -> {
                        if (result.value.succeeded) {
                            ...
                            navigateByAction(
                                R.id.action_postOTPDialogFragment_to_registrationSuccessFragment2,
                                bundle
                            )
                        } else {
                ...
                when (val result = registrationRepository.validateOTPForgot(it)) {
                    is Result.Success -> {
                        if (result.value.succeeded) {
                            ...
                            navigateByAction(
                                R.id.action_postOTPDialogFragment_to_postCreatePassFragment,
                                bundle
                            )
                        } else {
                            showErrorDialog(
