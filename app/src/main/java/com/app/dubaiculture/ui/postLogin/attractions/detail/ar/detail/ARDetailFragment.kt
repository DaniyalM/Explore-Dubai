...
class ARDetailFragment : BaseFragment<FragmentARDetailBinding>(), View.OnClickListener {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            arDetailViewModel.getMetaDataAr(it.getString(META_DATA_ID)!!,getCurrentLanguage().language)
        }
    ...
    private fun callingObserver() {
        arDetailViewModel.metaDataAr.observe(viewLifecycleOwner) {
            it?.let {
                ...
                binding.imgDetailPic.glideInstance(it.images!![0].image, isSvg = false).into(binding.imgDetailPic)
                rvSetUp()
                it.images.map {
                    groupAdapter.add(
                        ViewGalleryItems<ViewGalleryItemsBinding>(
                            object : RowClickListener {
                                override fun rowClickListener(position: Int) {
                                        arDetailViewModel.showToast(position.toString())
                                }
                            }, images = it,
